from training_testing_files.config import DEVICE, MATRIX_DIR, ACCLOSS_DIR, MODELS_DIR
...
def test(model, test_dataset, test_loader, criterion, num_epochs, selected_model, test_acc, test_loss, train_acc, train_loss):
    model.eval()
    ...
    for epoch in range(num_epochs):
        ...
        with open(ACCLOSS_DIR + "accuracy_auroc.json", "r") as file:
            data = json.load(file)

        cm = confusion_matrix(all_labels, all_preds)

        old_acc_value = data[selected_model]["acc"]
        old_auroc_value = data[selected_model]["auroc"]
        ...
        if accuracy > old_acc_value:
            ...
            final_path = MODELS_DIR + f"{selected_model}.pth"
        ...
        print(f"Epoch [{epoch + 1}/{num_epochs}], Test Loss: {epoch_loss:.4f}, Accuracy: {accuracy:.2f}%, AUROC: {auroc:.4f}")
        ...
        plt.tight_layout()
        # plt.savefig(ACCLOSS_DIR + f"{selected_model}_accuracy_loss.png", dpi=300)
        plt.close()
